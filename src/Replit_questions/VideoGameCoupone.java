package Replit_questions;

import java.util.Scanner;

public class VideoGameCoupone {
    public static void main(String[] args) {



        /*The video games machines at your local arcade outputs coupons according to how well you play the game.

        You can redeem 10 coupons for a candy bar or you can redeem 3 for a gumball

        Write a program that displays how many candy bars and gumballs you can get. You perfer candy bars.

        if you the coupons are not enough for any redeem, display message:

        "Not enough coupons"

        Example:

        Input:
        Enter number of coupons:
        13

        Output: Number of Candies: 1
        Output: Number of Gumballs: 1
        Input:
        Enter number of coupons:
        23

        Output: Number of Candies: 2
        Output: Number of Gumballs: 1
        Input:
        Enter number of coupons:
        2

        Output: Not enough coupons
                Console
        Shell
                Markdown*/
      //  Scanner scan=new Scanner(System.in);
       // System.out.println("Enter number of coupons:");
       // int numOfC= scan.nextInt();


     /*  int numOfCandies='';
        int numOfGumballs='';

        if(numOfC % 10==0 ){
            numOfCandies=numOfC/10;

            System.out.println("Number of Candies:"+ numOfCandies);*/
        Scanner scan = new Scanner(System.in);
        System.out.println("Enter number of coupons:");
        int coupons = scan.nextInt();
        int  nOfCandies= coupons/10;
        int nOfGumballs= (coupons%10)/3;
        int nOfGumballs2= coupons/3;

        if(coupons>=13){
            System.out.println("Number of Candies: "+nOfCandies);
            System.out.println("Number of Gumballs: "+nOfGumballs);
        }
        if (coupons>=10&&coupons<=12){
            System.out.println("Number of Candies: "+nOfCandies);
            System.out.println("Number of Gumballs: 0");
        }
        if (coupons>=3&&coupons<10){
            System.out.println("Number of Candies: 0");
            System.out.println("Number of Gumballs: "+nOfGumballs2);
        }
        if(coupons<=2){
            System.out.println("Not enough coupons");
        }



    }
}
